...
class StorePrice(models.Model):
    ...
    def integrity_check(self) -> bool:
        ...
        Validates whether the current hash matches the database state.
        """
        if not self.price_hash:
            return True
        secret_key = getattr(settings, 'SECRET_KEY', 'fallback_secret')
        raw_string = f"{self.current_price}-{self.store_name}-{self.last_updated.isoformat() if self.last_updated else ''}-{secret_key}"
    ...
    def save(self, *args, **kwargs) -> None:
        """
        SHA-256 Integrity Guards: Generates financial fingerprint to prevent tampering attacks.
        Since we are doing a legitimate application save (e.g. from the scraper), we always recalculate the hash based on the current time.
        """
        now = timezone.now()
        self.last_updated = now  # Force the timestamp to right now so our hash calculation matches what enters the DB
        
        secret_key = getattr(settings, 'SECRET_KEY', 'fallback_secret')
        raw_string = f"{self.current_price}-{self.store_name}-{now.isoformat()}-{secret_key}"
